...
let customerArr = ["Customer 1", "Customer 2", "Customer 3", "Customer 4", "Customer 5"];
let iconArr = ["fas fa-coffee", "fas fa-cookie", "fas fa-ice-cream", "fas fa-beer", "fas fa-hamburger"];
let totalOrder = [];
...
function createOrder() {
    ...
    if (result === 4) {
        order.push(random(customerArr), random(iconArr), random(iconArr), random(iconArr));
    }
...
function areaClick(evt) {
    ...
    document.getElementById("proposition").innerHTML += `<span>${clickedCustomer}</span>`;
}
...
function iconClick(evt) {
    ...
    document.getElementById("proposition").innerHTML += `<span class="${clickedIcon}"></span>`
}
...
document.getElementById("btn-delete").addEventListener("click", deleteProposition);

function loseLife() {
    life.pop();
    document.getElementsByClassName("life")[2 - life.length].classList.add("fa-heart-broken");
}
...
function check() {
    ...
    for (let i = 0; i < totalOrder.length; i++) {
        if (
            (JSON.stringify(propositionArr) == JSON.stringify(totalOrder[i])) == true) {
...
// A faire  :
// augmenter la rapidité de création des commandes au fur et à mesure des commandes servies
// remoduler le css pour page d'accueil / transition explication du jeu / jeu /feuille résultats)